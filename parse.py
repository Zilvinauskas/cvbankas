...
def main():
    ...
    for file in folder_path.glob("*.html"):
        ...
        salary = selector.xpath("//div[@class='label_component_body'][span[@class='data_tag_component_salary_amount']]/span[@class='data_tag_component_salary_amount']/text()").get()    
        
        # find out if after_tax and salary type
        parts = selector.xpath("//div[@class='label_component'][div[@class='label_component_body']]/div[@class='label_component_body']/text()").getall()
        joined_parts = "".join(parts).strip()        
        after_tax = "rankas" in joined_parts
        
        if "val." in joined_parts:
            salary_type = "hourly"
        else:
            salary_type = "monthly"  
